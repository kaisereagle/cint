 ...
 ************************************************************************
 * Source file libmake.c (makecint.c)
 ************************************************************************
 * Description:
...
{
  printf("################################################################\n");
  printf("# makecint : interpreter-compiler for cint (Borland C++ Builder version)\n");
  printf("#\n");
...
#ifdef G__BORLAND
/******************************************************************
* G__printuseunit_noopt()
******************************************************************/
void G__printuseunit_noopt(fp,list)
FILE *fp;
struct G__string_list *list;
{
  struct G__string_list *p;
  ...
  while(p) {
    ...
    default:
      if(p->string) {
        if(strstr(p->string,".cpp") || strstr(p->string,".CPP") ||
           strstr(p->string,".cxx") || strstr(p->string,".CXX") ||
           strstr(p->string,".C")   || strstr(p->string,".c")) {
          fprintf(fp,"USEUNIT(\"%s\");",p->string);
        }
        else if(strstr(p->string,".lib") || strstr(p->string,".LIB")) {
          fprintf(fp,"USELIB(\"%s\");",p->string);
        }
      }
      else if(p->object) {
        fprintf(fp,"USEOBJ(\"%s\");",p->object);
      }
      break;
    }
    p = p->next;
...
#ifdef G__BORLAND
  fprintf(fp,"# Borland C++ Builder specific #############################\n");
  fprintf(fp,"!if !$d(BCB)\n");
  fprintf(fp,"BCB = $(MAKEDIR)\..\n");
  fprintf(fp,"!endif\n");
  fprintf(fp,"VERSION = BCB.03\n");
  fprintf(fp,"\n");
  fprintf(fp,"# Target and option ########################################\n");
  fprintf(fp,"PROJECT = $(OBJECT)\n");
  fprintf(fp,"OBJFILES = G__bc_%s.obj $(CIFO) $(CPPIFO) $(COFILES) $(CPPOFILES) \n",G__DLLID);
  fprintf(fp,"LIBFILES = $(CINTLIB) $(LIBS)\n");
  fprintf(fp,"CFLAG2 = -D_RTLDLL;USEPACKAGES -I$(BCB)\\include;$(BCB)\\include\\vcl \\\n");
  fprintf(fp,"         $(IPATH) $(CINTIPATH) $(MACRO) -H=$(BCB)\\lib\\vcl35.csm \n");
  fprintf(fp,"\n");
  fprintf(fp,"# MAKEINF2 #################################################\n");

  /***************************************************************************
   * Copy platform dependent information from $CINTSYSDIR/MAKEINF2 
   ***************************************************************************/
  {
    char makeinf2[G__MAXFILENAME];
    FILE *makeinf2fp;
    sprintf(makeinf2,"%s/MAKEINF2",G__cintsysdir);
    makeinf2fp = fopen(makeinf2,"r");
    G__copyfile(fp,makeinf2fp);
    fclose(makeinf2fp);
  }
...
#if defined(G__BORLAND) || defined(G__DJGPP)
...
* G__outputborland
******************************************************************/
int G__outputborland() {
  char fname[G__MAXNAME];
  FILE* fp;

  sprintf(fname,"G__bc_%s%s",G__DLLID,G__CPPSRCPOST);
  fp = fopen(fname,"w");

  fprintf(fp,"/* Automatically generated by makecint */ \n");
  fprintf(fp,"#include <vcl.h>\n");
  fprintf(fp,"#pragma hdrstop\n");
  G__printuseunit_noopt(fp,G__CSRC);
  G__printuseunit_noopt(fp,G__CPPSRC);
  G__printuseunit_noopt(fp,G__LIB);
  fprintf(fp,"USELIB(\"%s\\libcint.lib\");\n",G__cintsysdir);
  fprintf(fp,"int WINAPI DllEntryPoint(HINSTANCE hinst, unsigned long reason, void*)\n");
  fprintf(fp,"{ return 1; }\n");
