...
#ifdef G__BORLANDCC5
int G__split(char *string,char *line,int *argc,char *argv[]);
...
void G__readMAKEINFO(void);
void G__freestringlist(struct G__string_list *list);
void G__cleanup(void);
char *G__replacepostfix(char *string, char *postfix, char *buf);
int G__ispostfix(char *string, char *postfix);
struct G__string_list* G__storestringlist(struct G__string_list *list, char *string);
void G__displayhelp(void);
int G__check(char *buf, char *item, char *where);
void G__printstringlist(FILE *fp, struct G__string_list *list, int mode);
void G__printstringlist_noopt(FILE *fp, struct G__string_list *list, int mode);
void G__printsourcecompile(FILE *fp, struct G__string_list *list, char *headers, char *compiler);
void G__printuseunit_noopt(FILE *fp, struct G__string_list *list);
void G__printtitle(void);
int G__readargument(int argc, char **argv);
void G__checksourcefiles(struct G__string_list *list,char *srcpost,char *objpost);
int G__checksetup(void);
void G__outputmakefile(int argc, char **argv);
void G__displaytodo(void);
...
{
  printf("################################################################\n");
  printf("# makecint : interpreter-compiler for cint (Borland C++ Compiler 5.5 version)\n");
  printf("#\n");
  ...
  while(i<argc) {
    ...
    *************************************************************************/
    ...
    /*************************************************************************/
...
#if defined(G__BORLAND) && !defined(G__BORLANDCC5)
      fprintf(stderr
	      ,"Limitation: -o option not supported for C++ Builder\n");
      fprintf(stderr
	      ,"            Please use -dl option instead\n");
#endif
      i++;
      ...
      G__isDLL = 0;
      mode = G__IDLE;
#if defined(G__DJGPP) || defined(G__BORLAND) || defined(G__WIN32)
...
char **argv;
{
  ...
  if(G__isDLL) {
    ...
    fprintf(fp,"\t$(LD) $(LDDLLOPT) $(OPTIMIZE) $(IPATH) $(MACRO) $(CCOPT) -e$(OBJECT) $(COFILES) $(CIFO) $(CPPIFO) $(CPPOFILES) $(LIBS) $(CINTSYSDIR)/libcint.lib\n");
  }
  else if(G__ismain) {
    fprintf(fp
   ,"$(OBJECT) : $(CINTLIB) $(READLINEA) G__init%s.obj $(COFILES) $(CPPOFILES) $(CIFO) $(CPPIFO) \n",G__DLLID);
    fprintf(fp,"\t$(LD) $(OPTIMIZE) $(IPATH) $(MACRO) $(CCOPT) -e$(OBJECT) $(CIFO) $(CPPIFO) $(COFILES) $(CPPOFILES) $(CINTLIB) G__init%s.obj $(READLINEA) $(LIBS) $(LDOPT)\n",G__DLLID);
  }
  else {
    fprintf(fp
   ,"$(OBJECT) : $(CINTLIB) $(READLINEA) G__main%s.obj $(COFILES) $(CPPOFILES) $(CIFO) $(CPPIFO) \n",G__DLLID);
    fprintf(fp,"\t$(LD) $(OPTIMIZE) $(IPATH) $(MACRO) $(CCOPT) -e$(OBJECT) $(CIFO) $(CPPIFO) $(COFILES) $(CPPOFILES) $(CINTLIB) G__main%s.obj $(READLINEA) $(LIBS) $(LDOPT)\n",G__DLLID);
  }
  fprintf(fp,"\n");
  ...
  if(!G__isDLL) {
    if(G__ismain) {
      fprintf(fp,"# Compile dictionary initialization class ################\n");
      fprintf(fp,"G__init%s.obj : G__init%s.cxx $(CINTSYSDIR)/G__ci.h\n",G__DLLID,G__DLLID);
      fprintf(fp,"\t$(CPP) $(LINKSPEC) $(CINTIPATH) $(OPTIMIZE) $(OPTION) -oG__init%s.obj -c G__init%s.cxx\n",G__DLLID,G__DLLID);
    }
    else {
      fprintf(fp,"# Compile dictionary setup main routine ##################\n");
      fprintf(fp,"G__main%s.obj : G__main%s.cxx $(CINTSYSDIR)/G__ci.h\n",G__DLLID,G__DLLID);
      fprintf(fp,"\t$(CPP) $(LINKSPEC) $(CINTIPATH) $(OPTIMIZE) $(OPTION) -oG__main%s.obj -c G__main%s.cxx\n",G__DLLID,G__DLLID);
    }
  }
  fprintf(fp,"\n");
  ...
  if(G__CHDR) {
    ...
    fprintf(fp,"\tcint %s -K -w%d -z%s -N%s -n$(CIFC) $(DLLSPEC) -D__MAKECINT__ %s -c-2 $(KRMODE) $(IPATH) $(MACRO) $(CINTOPT) $(CHEADERCINT)" 
	    ,G__INITFUNC,G__isDLL,G__DLLID,G__DLLID,G__preprocess);
  ...
  if(G__CPPHDR) {
    ...
    fprintf(fp,"\tcint %s -w%d -z%s -N%s -n$(CPPIFC) $(DLLSPEC) -D__MAKECINT__ %s -c-1 -A $(IPATH) $(MACRO) $(CINTOPT) $(CPPHEADERCINT)"
	    ,G__INITFUNC,G__isDLL,G__DLLID,G__DLLID,G__preprocess);
...
#ifdef G__BORLANDCC5
  fprintf(fp,"\t$(RM) %s.tds\n",G__DLLID);
  fprintf(fp,"\t$(RM) $(OBJECT)\n");
  if(G__CPPHDR) {
    fprintf(fp,"\t$(RM) $(CPPIFC)\n");
    fprintf(fp,"\t$(RM) $(CPPIFH)\n");
    fprintf(fp,"\t$(RM) $(CPPIFO)\n");
  }
  if(G__CHDR) {
    fprintf(fp,"\t$(RM) $(CIFC)\n");
    fprintf(fp,"\t$(RM) $(CIFH)\n");
  ...
  if(!G__isDLL) {
    if(G__ismain) 
      fprintf(fp,"\t$(RM) G__init%s%s\n",G__DLLID,G__OBJPOST);
    else
      fprintf(fp,"\t$(RM) G__main%s%s\n",G__DLLID,G__OBJPOST);
  }
#else
  if(G__isDLL) {
...
void G__outputmain()
{
  ...
  char fname[300];
  /*****************************************************************
  * creating G__main[DLLID].cxx or G__init[DLLID].cxx
  *****************************************************************/
  if(G__ismain) 
    sprintf(fname,"G__init%s.cxx",G__DLLID);
  else
    sprintf(fname,"G__main%s.cxx",G__DLLID);
  mainfp = fopen(fname,"w");
  fprintf(mainfp,"/******************************************************\n");
  fprintf(mainfp,"* %s\n",fname);
  ...
  if(G__ismain) {
    fprintf(mainfp,"class G__DMYp2fsetup%s {\n",G__DLLID);
    fprintf(mainfp," public:\n");
    fprintf(mainfp,"  G__DMYp2fsetup%s() { \n",G__DLLID);
    if(G__CHDR) fprintf(mainfp,"    G__set_p2fsetup(G__c_setup%s);\n",G__DLLID);
    if(G__CPPHDR) fprintf(mainfp,"    G__set_p2fsetup(G__cpp_setup%s);\n",G__DLLID);
    fprintf(mainfp,"  }\n");
    fprintf(mainfp,"  ~G__DMYp2fsetup%s() { G__free_p2fsetup(); }\n",G__DLLID);
    fprintf(mainfp,"} G__DMY%s;\n",G__DLLID);
  }
  else {
#endif
  ...
  fclose(mainfp);
  /*****************************************************************
  * end of creating G__main[DLLID].cxx or G__init[DLLID].cxx
  *****************************************************************/
}
#endif
