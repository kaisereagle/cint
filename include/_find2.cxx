...
void FindFiles(const char* fexpr) {
  ...
  //strset.setSimfunc(simfunc,IC_SIMFUNC_THREADUNSAFE);
  ...
  if(CLST==0 && regexp==0) {
	...
	strset.add(fullpath[j],fnameonly[j]);
    ...
    if(thresh<1.0) // clsutering by similarity threshold
      strset.clustering(thresh,2);
    else           // clustering by number of clusters
      strset.clustering(CLST,CLST*2,2);
	...
	int m;
	for(i=0;i<c.size();i++) {
	  m=e.match(c[i][1].c_str());
	  ct[i].let(j+1,m);
	}
      }
      ct.column_push_back(c.column(1));
      for(i=0;i<c.size();i++) {
	ct[i].let(t.size()+2,t[c[i][0].toint()][1].tostring());
	ct[i].let(t.size()+3,t[c[i][0].toint()][2].tostring());
      }
      ct.sort(0);
      ct.reverse();
    }
  ...
  if(prompt) {
    printf(" c:list of files, t:synthesized regular expression for clsuter, ct:verifycation");
    while(!G__pause());
  }
}
...
int main(int argc,char** argv) {
  for(int i=1;i<argc;i++) {
    ...
    else if(strcmp(argv[i],"-string")==0) simfunc=stringmatchratio;
