...
void G__ary_pulse(double* c,double* a,int n)
{
  int i;
  double pi2 = 2*PI;
  double dt = (a[1]-a[0])*1.1;
  double tmp;
  for(i=0;i<n;i++) {
    tmp = fmod(a[i],pi2);
    if(tmp<dt && (!i || c[i-1]==0)) c[i]= 1; 
    else {
      tmp = fmod(a[i]+PI,pi2);
      if(tmp<dt && (!i || c[i-1]==0)) c[i]= -1; 
    }
...
void G__ary_rand(double *c,double *a,int n)
{
  ...
  for(i=0;i<n;i++) c[i] = (double)(rand()%20000)/10000-1.0;
#else
