...
template<>
inline void Matrix2<DComplex>::print() {
  int i,j;
  for(j=0;j<Ysize;j++) {
    printf("j%-3d ",j);
    for(i=0;i<Xsize;i++) {
...
Matrix2<T> operator*(const Matrix2<T>& a,const Matrix2<T>& b) {
  ...
  int Xsize=Xsize2;
  int Ysize=Ysize1;
  if(Xsize1!=Ysize2 /*|| Ysize1!=Xsize2*/) {
    fprintf(stderr
	    ,"Error: Mastirx2<T>::operator*() size mismatch (%d,%d) (%d,%d)\n"
	    ,Xsize1,Ysize1,Xsize2,Ysize2);
    return(r);
  }
  r.Allocate(Xsize,Ysize);
  T* dat=r.Data();
  const T* data=a.Data();
  const T* datb=b.Data();
  T tmp;
  for(int i=0;i<Xsize;i++) {
    for(int j=0;j<Ysize;j++) {
      tmp =0.0; 
      for(int k=0;k<Xsize1;k++) {
        tmp = tmp + data[k*Ysize1+j]*datb[i*Ysize2+k];
      }
      dat[i*Ysize+j] = tmp;
    }
  }
...
template<class T>
int G__ateval(const Matrix1<T>& x) { x.disp(); return(1); }

template<class T>
int G__ateval(const Matrix2<T>& x) { x.disp(); return(1); }

template<class T>
int G__ateval(const Matrix3<T>& x) { x.disp(); return(1); }
