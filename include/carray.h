...
class carray  {
 public:
  ...
  int resize(int size);
  void push_back(double r,double i);
...
* push_back
***********************************************/
void array::push_back(double r,double i) {
  resize(n+1);
  re[n]=r;
  im[n]=i;
}
...
* rotate
***********************************************/
carray rotate(carray& a,double rad)
{
  carray c=carray(0.0 , 0.0 , a.n);
  int i;
  c.n=a.n;
  for(i=0;i<c.n;i++) {
    c.re[i]= a.re[i]*cos(rad) - a.im[i]*sin(rad);
    c.im[i]= a.re[i]*sin(rad) + a.im[i]*cos(rad);
  }
  return(c);
}
