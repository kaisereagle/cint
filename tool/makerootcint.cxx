/* /% C %/ */
/***********************************************************************
 * makeroocint (C/C++ interpreter-compiler)
 ************************************************************************
 * Source file makecint.c
 ************************************************************************
 * Description:
 *  This tool creates Makefile for encapsurating arbitrary C/C++ object
 * into ROOT as Dynamic Link Library or archived library
 ************************************************************************
...
{
  printf("################################################################\n");
  printf("# makecint : interpreter-compiler for rootcint\n");
  printf("#\n");
  printf("# Copyright(c) 2010~     Masaharu Goto (cint@pcroot.cern.ch)\n");
...
{
  printf("Usage :\n");
  printf(" makerootcint -mk [Makefile] -dl [DLL] -D[macro] -H [C++header] [LinkDef].h -C++ [C++source]\n");
...
#endif
    if(G__isrootcint) {
      fprintf(fp,"\trootcint -f $(CPPIFC) -c $(MACRO) $(CPPHEADERCINT)\n");
    }
    else {
#ifdef CINTSYSDIR
